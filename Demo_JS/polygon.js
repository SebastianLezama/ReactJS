const getAngle = (vertex, other) =>
  (Math.atan2(other[1] - vertex[1], other[0] - vertex[0]) * 180) / Math.PI;

const getCentroid = (polygon) =>
  polygon.reduce(
    (acc, vertex, _, { length }) => [
      acc[0] + vertex[0] / length,
      acc[1] + vertex[1] / length,
    ],
    [0, 0]
  );
const getLeftBottom = (polygon) =>
  polygon.sort((a, b) => (a[0] - b[0] !== 0 ? a[0] - b[0] : b[1] - a[1]))[0];
...
function sort(polygon) {
  ...
  return [...polygon].sort(
    (u, v) => getAngle(centroid, u) - getAngle(centroid, v)
  );
}
...
function drawLines(context, polygon) {
  ...
  for (const vertex of polygon) {
    ...
    context.fillText(
      `${Math.round(getAngle(centroid, vertex))}°`,
      avg(centroid[0], vertex[0]),
      avg(centroid[1], vertex[1])
    );
  }
  context.lineWidth = 2;
